...
@section('title', 'Edit Template Section')
...
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h5 class="h4">Edit Section: {{ $section->name }}</h5>
        ...
        <div class="col-md-8 mx-auto">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Edit Section Details</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.templates.sections.update', ['template' => $template->id, 'section' => $section->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3" id="column-layout-container">
                            ...
                            <select id="column_layout" name="column_layout" class="form-select column-layout-select @error('column_layout') is-invalid @enderror" {{ old('section_type', $section->section_type) !== 'multi-column' ? 'disabled' : '' }}>
                                <option value="">-- Select Layout --</option>
                                @foreach($columnLayouts as $value => $label)
                                    <option value="{{ $value }}" {{ old('column_layout', $section->column_layout) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3 form-check">
                            <input type="checkbox" id="is_repeatable" name="is_repeatable" class="form-check-input repeatable-checkbox @error('is_repeatable') is-invalid @enderror" value="1" {{ old('is_repeatable', $section->is_repeatable) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3" id="max-widgets-container" style="{{ old('is_repeatable', $section->is_repeatable) ? '' : 'display: none;' }}">
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="ri-save-line"></i> Update Section
                            </button>
                        </div>
                    </form>
                </div>
