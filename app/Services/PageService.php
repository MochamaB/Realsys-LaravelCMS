...
class PageService
{
    ...
    public function deletePage(Page $page): bool
    {
        ...
        try {
            ...
            /**
 * Synchronize page sections with template sections from the database
 * 
 ...
 */
    public function syncPageSections(Page $page): array
    {
        ...
        // Explicitly query the database for template sections
        $templateSections = \App\Models\TemplateSection::where('template_id', $templateId)
            ->orderBy('position')
            ->get();
        
        // Get existing page sections from database
        ...
        try {
            ...
            foreach ($templateSections as $templateSection) {
                // Check if this template section already has a corresponding page section
                ...
                if (!$pageSection) {
                    ...
                    \App\Models\PageSection::create([
                        ...
                        'column_span_override' => null, // Can be set by users later
                        'column_offset_override' => null, // Can be set by users later
                        'css_classes' => null, // Can be set by users later
                        'background_color' => null, // Can be set by users later
                        'padding' => null, // Can be set by users later
                        'margin' => null, // Can be set by users later
                    ]);
            ...
            foreach ($existingSections as $existingSection) {
                ...
                if (!$templateSection) {
                    // Delete the page section from the database
            ...
            \Illuminate\Support\Facades\Log::error('Failed to synchronize page sections: ' . $e->getMessage(), [
                'page_id' => $page->id,
